...
export class Task {
  ...
  async restoreCheckpoint(
    ...
    const messageIndex =
      this.clineMessages.findIndex((m) => m.ts === messageTs) - (offset || 0);
    ...
    if (!didWorkspaceRestoreFail) {
      ...
      if (restoreType !== "task") {
        ...
        const checkpointMessages = this.clineMessages.filter(
  ...
  async doesLatestTaskCompletionHaveNewChanges() {
    ...
    try {
      ...
      const lastTaskCompletedMessageCheckpointHash =
        lastTaskCompletedMessage?.lastCheckpointHash; // ask is only used to relinquish control, its the last say we care about
      // if undefined, then we get diff from beginning of git
      // if (!lastTaskCompletedMessage) {
      // 	console.error("No previous task completion message found")
      // 	return
  ...
  async *attemptApiRequest(previousApiReqIndex: number): ApiStream {
    ...
    try {
      ...
      const isOpenRouter =
        ...
        this.api instanceof ClineHandler;
  ...
  async presentAssistantMessage() {
    ...
    switch (block.type) {
      ...
      case "tool_use":
        ...
        const pushAdditionalToolFeedback = (
          feedback?: string,
          images?: string[]
        ) => {
          if (!feedback && !images) {
        ...
        switch (block.name) {
          ...
          case "replace_in_file": {
            ...
            const accessAllowed =
              this.clineIgnoreController.validateAccess(relPath);
            if (!accessAllowed) {
            ...
            try {
              ...
              if (block.partial) {
                ...
                if (!relPath) {
                  this.consecutiveMistakeCount++;
                  pushToolResult(
                    await this.sayAndCreateMissingParamError(block.name, "path")
                  );
                  await this.diffViewProvider.reset();
                ...
                await this.fileContextTracker.trackFileContext(
                  relPath,
                  "cline_edited"
                );
          ...
          case "read_file": {
            ...
            try {
              if (block.partial) {
                ...
                if (
                  this.shouldAutoApproveToolWithPath(
                    block.name,
                    block.params.path
                  )
                ) {
              ...
              if (block.partial) {
                ...
                if (action === "launch") {
                  ...
                  if (this.context) {
                    ...
                    console.warn(
                      "no controller context available for browserSession"
                    );
                  }
          ...
          case "execute_command": {
            ...
            try {
              if (block.partial) {
                ...
                const autoApproveResult = this.shouldAutoApproveTool(
                  block.name
                );
            ...
            try {
              if (block.partial) {
                ...
                if (text || images?.length) {
                  ...
                  pushToolResult(
                    formatResponse.toolResult(formatResponse.condense())
                  );
                  ...
                  await this.contextManager.triggerApplyStandardContextTruncationNoticeChange(
                    Date.now(),
                    await ensureTaskDirectoryExists(
                      this.getContext(),
                      this.taskId
                    )
                  );
  ...
  async recursivelyMakeClineRequests(
    ...
    const currentProviderId = (await getGlobalState(
      this.getContext(),
      "apiProvider"
    )) as string;
    ...
    const isFirstRequest =
      this.clineMessages.filter((m) => m.say === "api_req_started").length ===
      0;
    if (isFirstRequest) {
